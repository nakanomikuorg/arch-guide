...
1. 制作 Ventoy U 盘：
   ...
   ![ventoy-1](../static/advanced/make-install-disk/ventoy-1.png)

   ![ventoy-2](../static/advanced/make-install-disk/ventoy-2.png)
...
2. 拷贝镜像文件到镜像分区（卷标为 `Ventoy`）：
   ...
   ![ventoy-3](../static/advanced/make-install-disk/ventoy-3.png)
...
1. 类似 [7-1. 分区](../rookie/basic-install.md#_7-1-分区)，用 `cfdisk` 命令创建新分区。在 `Type` 中选择 `Microsoft 储存空间`（NTFS）：
   ...
   ![ventoy-4](../static/advanced/make-install-disk/ventoy-4.png)
...
2. 格式化分区：
   ...
   ![ventoy-5](../static/advanced/make-install-disk/ventoy-5.png)
...
1. 打开 Ventoy 基于浏览器的图形化操作界面：
   ...
   ![ventoy-6](../static/advanced/make-install-disk/ventoy-6.png)

2. 使用浏览器访问提示的本地端口即可：

   ![ventoy-7](../static/advanced/make-install-disk/ventoy-7.png)
...
1. 首先安装 Archiso 相关包：
   ...
   ![archiso-1](../static/advanced/make-install-disk/archiso-1.png)
...
2. 复制 `releng` 配置方案到合适位置：
   ...
   ![archiso-2](../static/advanced/make-install-disk/archiso-2.png)
...
![archiso-3](../static/advanced/make-install-disk/archiso-3.png)
...
1. 编辑 `pacman.conf` 配置文件：
   ...
   ![archiso-4](../static/advanced/make-install-disk/archiso-4.png)
...
2. 然后修改 `packages.x86_64`：
   ...
   ![archiso-5](../static/advanced/make-install-disk/archiso-5.png)
...
3. 可选向镜像里添加文件：
   ...
   ![archiso-6](../static/advanced/make-install-disk/archiso-6.png)
...
4. 配置登录管理器：
   ...
   1. 启用登录管理器的 systemd 服务来做到在引导时启动 X：
      ...
      ![archiso-8](../static/advanced/make-install-disk/archiso-8.png)
   ...
   2. 为了让图形环境在启动过程中自动启动，创建并编辑 `airootfs/root/customize_airootfs.sh`：
      ...
      ![archiso-9](../static/advanced/make-install-disk/archiso-9.png)
...
1. 创建工作目录和输出目录：
   ...
   ![archiso-10](../static/advanced/make-install-disk/archiso-10.png)
