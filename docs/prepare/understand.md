...
> 4. 我有一定的 Linux 基础，未来使用 archlinux 需要排错的时候如何寻找解决方案？

## archlinux 的历史

> 历史是现实的根源。反省过去，明晰当下，指导未来

- `2001 ~ 2002` —— 加拿大程序员兼吉他手 [Judd Vinet](https://github.com/jvinet) 从 2001 年初开始开发 archlinux，并在 2002 年 3 月 11 日正式发行 0.1 版。它受到 Slackware、BSD、PLD Linux 和 CRUX 的启发。但是那时候这些发行版缺少软件包管理器。所以 Judd Vinet 以同样的简洁原则建立发行版，并编写了 `pacman` 软件包管理器，自动处理软件包的安装、删除和更新
...
- `2012 ~ 2013` —— 2012 年 7 月发行的 archlinux 安装 ISO 映像开始放弃安装框架而转为使用不同的辅助脚本 [arch-install-scripts](https://archlinux.org/packages/extra/any/arch-install-scripts/)。并逐步开始将初始化系统迁移至 Systemd
...
但也正因为此配置 archlinux 相对于其它 Linux 发行版来说是繁琐。但繁琐是自由的代价。
...
- Manjaro Linux 的一些做法已经不符合 [arch 之道](<https://wiki.archlinux.org/title/Arch_Linux_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)>)
...
> 这一小节将讨论，当未来 archlinux 系统因为某些原因无法正常工作时，相较于其它 Linux 发行版，如何寻找解决方案、以及有哪些特殊的排错技巧。注意，这只是非常概括的介绍，更多相关信息请参阅指南[常见问题排除与解决](../advanced/debug)章节
