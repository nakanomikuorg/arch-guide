...
2. 打开安卓设备的 `设置` > `开发人员选项`（多次点击 `关于手机（平板）` 中的 `版本号`，提示处在 `开发者模式` 后即可在设置中找到）> 打开 `USB 调试`：
   ...
   ![scrcpy-1](../static/play-and-office/collaboration/scrcpy-1.jpg)
...
1. 保持数据线连接，直接在终端执行以下命令即可：
   ...
   ![scrcpy-4](../static/play-and-office/collaboration/scrcpy-4.png)

   ![scrcpy-5](../static/play-and-office/collaboration/scrcpy-5.png)
...
5. 连接成功后在终端执行以下命令即可看到现在的安卓设备界面了：
   ...
   ![scrcpy-8](../static/play-and-office/collaboration/scrcpy-8.png)
